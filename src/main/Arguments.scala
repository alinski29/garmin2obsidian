...
case class Arguments(
    ...
    exportConfigPath: Option[os.Path] = Some(os.RelPath(".GarminDb/GarminConnectConfig.json").resolveFrom(os.home)),
...
object Arguments:
  def apply(args: String*): Try[Arguments] =
    OParser.parse(parser, args, Arguments()) match
      case Some(opts) =>
        if !opts.sourceDir.toIO.isDirectory() then Failure(new Exception(s"${opts.sourceDir} is not a directory"))
        if !opts.exportConfigPath.get.toIO.isFile() then
          Failure(new FileNotFoundException(s"Missing export configuration file at ${opts.exportConfigPath}"))
        else Success(opts)
      case _ =>
